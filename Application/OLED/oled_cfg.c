...
UINT8_T OLED_Device0_Init(OLED_IIC_HandlerType *OLEDx)
{
	//---RST端口的配置
	OLEDx->msgRstBit = 0;
	OLEDx->msgRstPort = NULL;

	//---端口初始化
	if (OLEDx->msgRstPort != NULL)
	{
		GPIOTask_Clock(OLEDx->msgRstPort, 1);
	}
	...
	//---RST端口的初始化
	GPIO_InitStruct.Pin = OLEDx->msgRstBit;
	if (OLEDx->msgRstPort != NULL)
	{
		LL_GPIO_Init(OLEDx->msgRstPort, &GPIO_InitStruct);
		GPIO_OUT_1(OLEDx->msgRstPort, OLEDx->msgRstBit);
	}

	//---I2C的配置
	OLEDx->msgI2C.msgI2Cx = NULL;
	OLEDx->msgI2C.msgSclPort = GPIOB;
	OLEDx->msgI2C.msgSdaPort = GPIOB;
	OLEDx->msgI2C.msgSclBit = LL_GPIO_PIN_8;
	OLEDx->msgI2C.msgSdaBit = LL_GPIO_PIN_9;
...
UINT8_T OLED_I2C_Init(OLED_IIC_HandlerType *OLEDx, void(*msgDelay)(UINT32_T delay), UINT8_T isHWI2C)
{
	...
	{
		//---初始化软件模拟I2C
		_return = I2CTask_MSW_Init(&(OLEDx->msgI2C), msgDelay);
		OLEDx->msgI2C.msgModelIsHW = 0;
	}
...
void OLED_I2C_HWInit(OLED_IIC_HandlerType *OLEDx)
{
	if (OLEDx->msgRstPort != NULL)
	{
		GPIO_OUT_1(OLEDx->msgRstPort, OLEDx->msgRstBit);
		DelayTask_ms(100);
		GPIO_OUT_0(OLEDx->msgRstPort, OLEDx->msgRstBit);
		DelayTask_ms(200);
		GPIO_OUT_1(OLEDx->msgRstPort, OLEDx->msgRstBit);
	}
