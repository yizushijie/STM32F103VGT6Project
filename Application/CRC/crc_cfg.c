...
#if defined(USE_CRC32)&&!defined(CRC)
...
#if defined(USE_MCU_AVR)&&!defined(USE_SRAM)
PRO_FLASH_UINT8_T  crc8Table07[256] =
#else
const UINT8_T  crc8Table07[256] =
#endif
{
...
#if defined(USE_MCU_AVR)&&!defined(USE_SRAM)
PRO_FLASH_UINT8_T  crc8Table31[256] =
#else
const UINT8_T  crc8Table31[256] =
#endif
{
...
//---0--使能，0---使能
VLTUINT8_T g_CRCIsEnable = ERROR_1;
...
void CRC_Init(void)
{
	...
	#endif
	#if defined(USE_CHECKSUM)||defined(USE_CRC8)||defined(USE_CRC16)||defined(USE_CRC32)
	g_CRCIsEnable = OK_0;
	#endif
}
...
{
	#ifdef CRC
	return CRC_CRC32_HWCRC(pVal, length);
	#elif defined(USE_CRC32)
	return CRC_CRC32_Table(pVal, length);
	#else
	return 0
		#endif
}
...
UINT16_T CRC_CRC16_Table(UINT8_T *pVal, UINT32_T length)
{
	...
	for (index = 0; index < length; index++)
	{
		nTemp = (UINT8_T)((nReg >> 8) ^ pVal[index]);
		nReg = (nReg << 8) ^ crc16Table[nTemp];
	}
	return nReg;
	#else
	return 0;
	...
	for (index = 0; index < length; index++)
	{
		...
		switch (crcVal)
		{
			case USE_CRC8_07H:
				...
				//---为了减少AVR芯片SRM的占用
				#if defined(USE_MCU_AVR)&&!defined(USE_SRAM)
				nReg = PGM_READ_BYTE(crc8Table07 + nTableIndex);
				#else
				nReg = crc8Table07[nTableIndex];
				#endif
				break;
			case USE_CRC8_31H:
				...
				//---为了减少AVR芯片SRM的占用
				#if defined(USE_MCU_AVR)&&!defined(USE_SRAM)
				nReg = PGM_READ_BYTE(crc8Table31 + nTableIndex);
				#else
				nReg = crc8Table31[nTableIndex];
				#endif
				break;
			default:
				break;
