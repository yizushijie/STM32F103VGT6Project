...
//===变量的定义
SHT2X_HandlerType g_SHT2XDevice0;
...
UINT8_T SHT2X_I2C_Init(SHT2X_HandlerType *SHT2x, void(*msgDelay)(UINT32_T delay), UINT8_T isHWI2C)
{
	...
	{
		//---初始化软件模拟I2C
		_return = I2CTask_MSW_Init(&(SHT2x->msgI2C), msgDelay);
...
UINT8_T SHT2X_I2C_Device0_Init(SHT2X_HandlerType *SHT2x)
{
	...
	SHT2x->msgI2C.msgSclPort = GPIOB;
	SHT2x->msgI2C.msgSdaPort = GPIOB;
	SHT2x->msgI2C.msgSclBit = LL_GPIO_PIN_8;
	SHT2x->msgI2C.msgSdaBit = LL_GPIO_PIN_9;
	SHT2x->msgI2C.msgModelIsHW = 0;
	SHT2x->msgI2C.msgPluseWidth = 0;
	SHT2x->msgI2C.msgFuncDelay = NULL;
...
{
	return SHT2X_I2C_WriteCmd(SHT2x, SOFT_RESET_DEVICE);
}
...
UINT8_T SHT2X_I2C_ReadUserReg(SHT2X_HandlerType *SHT2x, UINT8_T *pReg)
{
	...
	//---发送寄存器命令
	I2CTask_MSW_SendByte(&(SHT2x->msgI2C), USER_READ_REG);
...
UINT8_T SHT2X_I2C_WriteUserReg(SHT2X_HandlerType *SHT2x, UINT8_T reg)
{
	...
	//---发送寄存器命令
	I2CTask_MSW_SendByte(&(SHT2x->msgI2C), USER_WRITE_REG);
...
UINT8_T SHT2X_I2C_ReadTempPoll(SHT2X_HandlerType *SHT2x)
{
	...
	//---读取温度值
	_return = SHT2X_I2C_ReadPoll(SHT2x, &temp, TRIG_TEMP_MEASUREMENT_POLL);
...
UINT8_T SHT2X_I2C_ReadHumiPoll(SHT2X_HandlerType *SHT2x)
{
	...
	//---读取湿度值
	_return = SHT2X_I2C_ReadPoll(SHT2x, &temp, TRIG_HUMI_MEASUREMENT_POLL);
...
UINT8_T SHT2X_I2C_ReadHM(SHT2X_HandlerType *SHT2x, UINT16_T *pVal, UINT8_T cmd)
{
	...
	//---等待从机释放SCL数据线
	while (GPIO_GET_STATE(SHT2x->msgI2C.msgSclPort, SHT2x->msgI2C.msgSclBit) ==
		0)
	{
		timeout++;
...
UINT8_T SHT2X_I2C_ReadTempHM(SHT2X_HandlerType *SHT2x)
{
	...
	//---读取温度值
	_return = SHT2X_I2C_ReadHM(SHT2x, &temp, TRIG_TEMP_MEASUREMENT_HM);
...
UINT8_T SHT2X_I2C_ReadHumiHM(SHT2X_HandlerType *SHT2x)
{
	...
	//---读取湿度值
	_return = SHT2X_I2C_ReadPoll(SHT2x, &temp, TRIG_HUMI_MEASUREMENT_HM);
